...
- (UMDiameterPacket *)createRAR
{
    ...
    avp_auth_app_id.number = self.authAppId;
    ...
    if(self.userName.length > 0)
    {
        ...
        avp_user_name.value = self.userName;
        [packet appendAvp:avp_user_name];
    }
    if(self.originStateId )
    {
        UMDiameterAvpOriginStateId *avp_orign_state_id = [[UMDiameterAvpOriginStateId alloc]init];
        avp_orign_state_id.number = self.originStateId;
        [packet appendAvp:avp_orign_state_id];
    }

    NSInteger n = _proxyInfos.count;
    if(n > 0)
    {
        for(NSInteger i=0;i<n;i++)
        {
            UMDiameterAvpProxyInfo *pi = _proxyInfos[i];
            [packet appendAvp:pi];

        }
    }
    n = _routeRecords.count;
    if(n > 0)
    {
        for(NSInteger i=0;i<n;i++)
        {
            UMDiameterAvpDiameterIdentity *di = _routeRecords[i];
            [packet appendAvp:di];
        }
    }
    ...
    if( n  > 0)
    {
        for(NSInteger i=0;i<n;i++)
        {
            UMDiameterAvp *avp = _optional_AVPs[i];
