...
//  ulibdiameter
//
//  Created by afink on 2019-06-30 13:20:49.332000
...
- (void)parseDict:(NSDictionary *)d
            error:(NSError **)eptr
{
    if(d["supported-services"])
    {
        id obj = d["supported-services"];
        if([obj isKindOfClass:[NSString class]])
        {
            _supported_services = [[UMDiameterAvpSupported_Services alloc]initWithString:(NSString *)obj];
        }
        if([obj isKindOfClass:[NSDictionary class]])
        {
            _supported_services = [[UMDiameterAvpSupported_Services alloc]initWithDictionary:(NSDictionary *)obj];
        }
    }
    if(d["route-record"])
    {
        id obj = d["route-record"];
        NSArray *a;
        if([obj isKindOfClass:[NSString class]])
        {
            a = [(NSString *)obj componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]];
        }
        if([obj isKindOfClass:[NSArray class]])
        {
            a = (NSArray *)a;
        }
        NSMutableArray<UMDiameterAvpRoute_Record *> *b = [[NSMutableArray alloc]init];
        for(NSString *obj in a)
        {
            UMDiameterAvpRoute_Record *avp
            if([obj isKindOfClass:[NSString class]])
            {
                avp = [[UMDiameterAvpRoute_Record alloc]initWithString:(NSString *)obj];
                [b addObject:avp];
            }
            else if([obj isKindOfClass:[NSDictionary class]])
            {
                avp = [[UMDiameterAvpRoute_Record alloc]initWithDictionary:(NSDictionary *)obj];
                [b addObject:avp];
            }
        }
        _route_record = b;
...
@try
{
    result = [parser objectWithString:s];
}
