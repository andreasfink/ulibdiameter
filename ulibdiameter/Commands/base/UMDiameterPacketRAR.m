...
//  ulibdiameter
//
//  Created by afink on 2021-03-23 03:34:00.193799
...
- (void)afterDecode
{
    ...
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        else if(avp.avpCode == [UMDiameterAvpRe_Auth_Request_Type avpCode])
        {
            UMDiameterAvpRe_Auth_Request_Type *avp2 = [[UMDiameterAvpRe_Auth_Request_Type alloc]initWithAvp:avp];
            _var_re_auth_request_type = avp2;
        }
...
+ (NSString *)webJsonDefintion
{
    ...
    {
        UMSynchronizedSortedDictionary *def =  [UMDiameterAvpRe_Auth_Request_Type definition];
        def[@"multiple"]=@(NO);
        def[@"mandatory"]=@(YES);
        [arr addObject:def];
    }
    ...
    commandDef[@"command-name"] = @"RAR";
    commandDef[@"web-name"] = @"rar";
