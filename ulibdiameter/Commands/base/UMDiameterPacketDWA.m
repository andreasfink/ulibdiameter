...
//  ulibdiameter
//
//  Created by afink on 2021-03-22 08:33:27.596992
...
- (void)afterDecode
{
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        if(avp.avpCode == [UMDiameterAvp@ avpCode])
        {
                UMDiameterAvp *avp2 = [[_var_failed_avpUMDiameterAvp alloc]initWithAvp:avp];
            (null) = avp2;
            [knownAVPs addObject:avp2]
        }
        if(avp.avpCode == [UMDiameterAvp@ avpCode])
        {
                UMDiameterAvp *avp2 = [[_var_origin_state_idUMDiameterAvp alloc]initWithAvp:avp];
            (null) = avp2;
            [knownAVPs addObject:avp2]
        }
        else
