...
//  ulibdiameter
//
//  Created by afink on 2021-03-22 08:41:00.135951
...
- (void)afterDecode
{
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        if(avp.avpCode == [UMDiameterAvpLRA_Flags avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpLRA_Flags alloc]initWithAvp:avp];
            _var_lra_flags = avp2;
            [knownAVPs addObject:avp2]
        }
        if(avp.avpCode == [UMDiameterAvpReporting_PLMN_List avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpReporting_PLMN_List alloc]initWithAvp:avp];
            _var_reporting_plmn_list = avp2;
            [knownAVPs addObject:avp2]
        }
...
+ (NSString *)webJsonDefintion
{
    ...
    {
        UMSynchronizedSortedDictionary *def =  [UMDiameterAvpLRA_Flags definition];
        def[@"multiple"]=@(NO);
        def[@"mandatory"]=@(NO);
        [arr addObject:def];
    }
    {
        UMSynchronizedSortedDictionary *def =  [UMDiameterAvpReporting_PLMN_List definition];
        def[@"multiple"]=@(NO);
        def[@"mandatory"]=@(NO);
        [arr addObject:def];
    }
