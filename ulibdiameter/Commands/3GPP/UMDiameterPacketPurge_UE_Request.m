...
//  ulibdiameter
//
//  Created by afink on 2021-03-22 08:40:59.944409
...
- (void)setDictionaryValue:(NSDictionary *)dict
{
    ...
    if(dict[@"pur-flags"])
    {
        _var_pur_flags = [[UMDiameterAvpPUR_Flags alloc]init];
        _var_pur_flags.objectValue = dict[@"pur-flags"];
    }
...
- (UMSynchronizedSortedDictionary *)dictionaryValue
{
    ...
    if(_var_pur_flags)
    {
        dict[@"pur-flags"] = _var_pur_flags.objectValue;
    }
...
+ (void)webDiameterParameters:(NSMutableString *)s
{
    ...
    [UMDiameterAvpPUR_Flags appendWebDiameterParameters:s webName:@"pur-flags"  comment:@"" css:@"optional"];
...
- (void)afterDecode
{
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        if(avp.avpCode == [UMDiameterAvpPUR_Flags avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpPUR_Flags alloc]initWithAvp:avp];
            _var_pur_flags = avp2;
            [knownAVPs addObject:avp2]
        }
...
+ (NSString *)webJsonDefintion
{
    ...
    {
        UMSynchronizedSortedDictionary *def =  [UMDiameterAvpPUR_Flags definition];
        def[@"multiple"]=@(NO);
        def[@"mandatory"]=@(NO);
        [arr addObject:def];
    }
    ...
    commandDef[@"web-name"] = @"purge-ue-request";
