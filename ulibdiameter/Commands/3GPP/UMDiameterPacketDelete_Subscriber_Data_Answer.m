...
//  ulibdiameter
//
//  Created by afink on 2021-03-22 08:40:59.650727
...
- (void)afterDecode
{
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        if(avp.avpCode == [UMDiameterAvpDSA_Flags avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpDSA_Flags alloc]initWithAvp:avp];
            _var_dsa_flags = avp2;
            [knownAVPs addObject:avp2]
        }
...
+ (NSString *)webJsonDefintion
{
    ...
    {
        UMSynchronizedSortedDictionary *def =  [UMDiameterAvpDSA_Flags definition];
        def[@"multiple"]=@(NO);
        def[@"mandatory"]=@(NO);
        [arr addObject:def];
    }
    ...
    commandDef[@"command-name"] = @"Delete-Subscriber-Data-Answer";
    commandDef[@"web-name"] = @"delete-subscriber-data-answer";
