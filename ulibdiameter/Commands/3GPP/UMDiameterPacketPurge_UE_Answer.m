...
//  ulibdiameter
//
//  Created by afink on 2021-03-22 08:40:59.912539
...
- (void)afterDecode
{
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        if(avp.avpCode == [UMDiameterAvpPUA_Flags avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpPUA_Flags alloc]initWithAvp:avp];
            _var_pua_flags = avp2;
            [knownAVPs addObject:avp2]
        }
...
+ (NSString *)webJsonDefintion
{
    ...
    {
        UMSynchronizedSortedDictionary *def =  [UMDiameterAvpPUA_Flags definition];
        def[@"multiple"]=@(NO);
        def[@"mandatory"]=@(NO);
        [arr addObject:def];
    }
