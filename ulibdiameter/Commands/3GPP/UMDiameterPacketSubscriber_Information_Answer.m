//
//  UMDiameterPacketSubscriber_Information_Answer.m
//  ulibdiameter
//
//  Created by afink on 2021-03-26 20:57:25.891631
//  Copyright © 2019 Andreas Fink. All rights reserved.
//


#import "UMDiameterPacketSubscriber_Information_Answer.h"
...
#import "UMDiameterAvpService_Data.h"
#import "UMDiameterAvpFailed_AVP.h"
#import "UMDiameterAvpProxy_Info.h"
#import "UMDiameterAvpRoute_Record.h"
#import "UMDiameterAvpAVP.h"

@implementation UMDiameterPacketSubscriber_Information_Answer
...
- (void)beforeEncode
{
    ...
    if(_var_user_identifier.count > 0)
    {
        for(UMDiameterAvpUser_Identifier *o in _var_user_identifier)
        {
            [o beforeEncode];
            [arr addObject:o];
        }
    }
    if(_var_service_data)
    {
        [_var_service_data beforeEncode];
        [arr addObject:_var_service_data];
    }
    if(_var_failed_avp)
    {
...
- (void)setDictionaryValue:(NSDictionary *)dict
{
    ...
    {
        id obj = dict[@"user-identifier"];
        if([obj isKindOfClass:[NSArray class]])
        {
            NSMutableArray *arr = [[NSMutableArray alloc]init];
            for(id entry in (NSArray *)obj)
            {
                UMDiameterAvpUser_Identifier *o = [[UMDiameterAvpUser_Identifier alloc]init];
                o.objectValue = entry;
                [arr addObject:o];
            }
            _var_user_identifier = arr;
        }
        else
        {
            NSMutableArray *arr = [[NSMutableArray alloc]init];
    ...
    if(dict[@"service-data"])
    {
        _var_service_data = [[UMDiameterAvpService_Data alloc]init];
        _var_service_data.objectValue = dict[@"service-data"];
    }
...
- (UMSynchronizedSortedDictionary *)dictionaryValue
{
    ...
    {
        NSMutableArray *arr = [[NSMutableArray alloc]init];
        for(id entry in _var_user_identifier)
        {
            [arr addObject:[entry objectValue]];
        }
        dict[@"user-identifier"] = arr;
    }
    if(_var_service_data)
    {
        dict[@"service-data"] = _var_service_data.objectValue;
    }
    if(_var_failed_avp)
    {
...
+ (void)webDiameterParameters:(NSMutableString *)s
{
    ...
    [UMDiameterAvpService_Data appendWebDiameterParameters:s webName:@"service-data"  comment:@"" css:@"optional"];
...
- (void)afterDecode
{
    ...
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        else if(avp.avpCode == [UMDiameterAvpUser_Identifier avpCode])
        {
            ...
            if(_var_user_identifier == NULL)
            {
                _var_user_identifier = (NSArray<UMDiameterAvpUser_Identifier *>*)@[avp2];
            }
            else
            {
                _var_user_identifier = [_var_user_identifier arrayByAddingObject:avp2];
            }
        }
        else if(avp.avpCode == [UMDiameterAvpService_Data avpCode])
        {
            UMDiameterAvpService_Data *avp2 = [[UMDiameterAvpService_Data alloc]initWithAvp:avp];
            _var_service_data = avp2;
        }
...
- (id)objectValue
{
	...
	}
	{
		...
		if(_var_user_identifier.count>0)
		{
			for(UMDiameterAvp *avp in _var_user_identifier)
			{
				[arr addObject:[avp objectValue]];
			}
			dict[@"User-Identifier"] = arr;
...
+ (NSString *)webJsonDefintion
{
    ...
    {
        UMSynchronizedSortedDictionary *def =  [UMDiameterAvpService_Data definition];
        def[@"multiple"]=@(NO);
        def[@"mandatory"]=@(NO);
        [arr addObject:def];
    }
    ...
    commandDef[@"command-name"] = @"Subscriber-Information-Answer";
    commandDef[@"web-name"] = @"subscriber-information-answer";
