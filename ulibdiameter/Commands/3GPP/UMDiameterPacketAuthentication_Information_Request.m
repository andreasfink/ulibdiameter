...
//  ulibdiameter
//
//  Created by afink on 2021-03-22 08:40:59.487427
...
- (void)afterDecode
{
    for(UMDiameterAvp *avp in _packet_avps)
    {
        ...
        if(avp.avpCode == [UMDiameterAvpRequested_EUTRAN_Authentication_Info avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpRequested_EUTRAN_Authentication_Info alloc]initWithAvp:avp];
            _var_requested_eutran_authentication_info = avp2;
            [knownAVPs addObject:avp2]
        }
        if(avp.avpCode == [UMDiameterAvpRequested_UTRAN_GERAN_Authentication_Info avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpRequested_UTRAN_GERAN_Authentication_Info alloc]initWithAvp:avp];
            _var_requested_utran_geran_authentication_info = avp2;
            [knownAVPs addObject:avp2]
        }
        ...
        if(avp.avpCode == [UMDiameterAvpAIR_Flags avpCode])
        {
                UMDiameterAvp *avp2 = [[UMDiameterAvpAIR_Flags alloc]initWithAvp:avp];
            _var_air_flags = avp2;
            [knownAVPs addObject:avp2]
        }
