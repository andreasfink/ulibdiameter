...
- (void)afterDecode
{
    ...
    for(UMDiameterAvp *avp in avps)
    {
        ...
        {
            avp = [[UMDiameterAvpCall_ID_SIP_Header alloc]initWithAvp:avp];
            _var_call_id_sip_header = avp;
            [knownAVPs addObject:avp];
        }
        else if(avp.avpCode == [UMDiameterAvpTo_SIP_Header avpCode])
        {
            avp = [[UMDiameterAvpTo_SIP_Header alloc]initWithAvp:avp];
            _var_to_sip_header = avp;
            [knownAVPs addObject:avp];
        }
        else if(avp.avpCode == [UMDiameterAvpRecord_Route avpCode])
        {
            avp = [[UMDiameterAvpRecord_Route alloc]initWithAvp:avp];
            _var_record_route = avp;
            [knownAVPs addObject:avp];
        }
        else
        {
